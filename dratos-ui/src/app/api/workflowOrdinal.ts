...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === 'POST') {
        const { action, workflowData, ownerPublicKey, newOwnerAddress } = req.body
        ...
        try {
            ...
            if (action === 'deploy') {
                ...
                res.status(200).json({ success: true, instance })
                    ...
                    const instance = await transferWorkflowOrdinal(
                    workflowData, ownerPublicKey.privateKey, newOwnerAddress)
                res.status(200).json({ success: true, instance })
            } else {
